...
Each feature is decomposed into four slices executed in a dependency order that enables parallelism:
...
|-------|-------------|---------------|-----------------|
| Contract | Extracts TypeScript types from tests | TypeScript compiler (types check) | No — runs first |
| API | Implements backend routes, services, models | Vitest + Supertest unit tests | Yes — parallel with Web |
| Web | Implements frontend pages, components | Component tests, Next.js build | Yes — parallel with API |
...
3.  Re-validate by running the test suite for the current slice:
      a. Contract slice: verify type files compile (tsc --noEmit).
      b. API slice: run unit tests for the slice's testFiles paths.
      c. Web slice: run component tests or build check.
      d. Integration slice: run Cucumber + Playwright for the slice's testFiles.
...
**Key principle:** The `features[]` array in `state.json` contains everything needed — per-slice test file paths, dependency order, failing tests with error messages, modified files. No prior session memory is required.

## 2d. Contract Generation (first step per feature)

Before implementing any code for a feature, generate the **contract types** — TypeScript interfaces that define the API surface between backend and frontend. These types are the stable bridge that enables parallel API and Web slice execution.

```
1.  Read the Gherkin scenarios for this feature (specs/features/{feature}.feature).
2.  Read ALL test files for this feature — step definitions, unit tests, e2e specs.
3.  Extract the concrete API surface from these test files:
      a. API endpoints: HTTP methods, URL paths, query parameters
      b. Request body shapes: field names, types, required vs. optional
      c. Response body shapes: status codes, field names, types
      d. Component props: what data the UI expects from the API
      e. Error shapes: error codes, messages, validation errors
4.  Generate TypeScript interface files in src/shared/types/{feature}.ts:
      - Request types: {Feature}Request, {Feature}CreateRequest, etc.
      - Response types: {Feature}Response, {Feature}ListResponse, etc.
      - Error types: {Feature}Error, etc.
      - Shared enums and constants
5.  Verify the types compile: tsc --noEmit
6.  Update state.json: set slices.contract.status = "done",
    record outputFiles.
```

### Contract Rules

- Types are **extracted from tests, not invented**. Every field in the contract must correspond to an assertion or API call in a test file.
- Contracts are **minimal** — only what the tests reference. Do not add speculative fields.
- Contracts are **the only shared code** between API and Web slices. Both slices import from `src/shared/types/`.
- If a test references a field not in the contract, add it. If no test references a field, do not add it.

## 3. API Slice: Backend Implementation

After the contract is generated, implement the backend. This slice runs independently — no browser or frontend needed.
...
- Request/response types MUST import from `src/shared/types/{feature}.ts` — the contract types.
- If a test expects a response shape that differs from the contract, the contract is wrong — update the contract first, then implement.

## 3b. Web Slice: Frontend Implementation

After the contract is generated, implement the frontend. This slice MAY run in parallel with the API slice — it mocks API calls using the contract types.
...
- A feature is only `"done"` when ALL four slices (contract + api + web + integration) are green.
...
1. Update `.spec2cloud/state.json`:
   ...
   - If all four slices are done, set the feature's `status` to `"done"`.
